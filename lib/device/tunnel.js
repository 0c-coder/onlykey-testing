 ...
 *   the first request.
 *
 *   DERIVATION. okcrypto_hkdf() reads the rpId out of the request, so web-derived
 *   P-256 / Curve25519 / NaCl keys are bound to it: a different rpId derives a
 *   different key. Derived X-Wing is NOT - its seed has no origin in it at all
 *   (seed/v3), which is what lets the CLI and every origin agree.
 *
...
/** A production origin: admitted by webcryptcheck() on every build. */
